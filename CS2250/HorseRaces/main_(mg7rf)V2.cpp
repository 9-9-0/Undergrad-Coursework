	...
	while (playAgain == true && winnerFound == false)
	{
		...
		*/
		...
		if (numWinners > 1)																//If loop in case multiple winners are found
		{
			...
			for (int searchDCounter = 0; searchDCounter < numHorses; searchDCounter++)
			{
				if (horsePtr[searchDCounter]->getHorseDistance() >= raceDistance)
				{
					if (horsePtr[searchDCounter]->getHorseDistance() >= greatestDistance)
					{
						greatestDistance = horsePtr[searchDCounter]->getHorseDistance();
					}
				}
			}

			for (int poolCounter = 0; poolCounter < numHorses; poolCounter++)
			{
				if (horsePtr[poolCounter]->getHorseDistance() == greatestDistance)
				{
